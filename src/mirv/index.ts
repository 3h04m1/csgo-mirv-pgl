...
import { GameEventUnserializer, Result } from "../gameEvents/gameEvent";
  ...
  for (const eventName in enrichments) {
    ...
    for (const keyName in enrichments[eventName]) {
      ...
      for (let i = 0; i < arrEnrich.length; ++i) {
        sendCommand(
          ws,
          `mirv_pgl events enrich eventProperty "${arrEnrich[i]}" "${eventName}" "${keyName}"`
        );
      }
    }
  }
 ...
 * 
*/
export function parseMIRV(data: RawData, ws: WebSocket):Result {
